    ...
    >
        ...
        {% for field in form.fields %}
            ...
            {% elseif field.type == 'textarea' %}
                <div class="form-group field-{{ field.code }}">
                    <label for="{{ field.code }}">{{ field.title }}</label>
                    <textarea class="form-control" name="{{ field.code }}" id="{{ field.code }}" placeholder="{{ field.placeholder }}" rows="7"></textarea>
                </div>

            {% elseif field.type == 'select' %}
                <div class="form-group field-{{ field.code }}">
                    <label for="{{ field.code }}">{{ field.title }}</label>
                    <select class="form-control" name="{{ field.code }}" id="{{ field.code }}">
                    ...
                    <script>
                        grecaptcha.ready(function () {
                            grecaptcha.execute('{{ recaptcha.site_key }}', {action: '{{ recaptcha.action|default('submit') }}'}).then(function (token) {
                                const form = document.getElementById('form-{{ form.code }}');
                                if (form) {
                ...
                <div class="form-group field-radio">
                    {% for option in field.options %}
                        <label class="radio-inline">
                            <input type="radio" name="{{ field.code }}" value="{{ option.value }}" id="{{ field.code ~ '_' ~ loop.index }}">
                            {{ option.title }}
                        </label>
                    {% endfor %}
                </div>
            ...
            {% elseif field.type == 'checkbox' %}
                <div class="form-group form-check field-{{ field.code }}">
                    <input class="form-check-input" type="checkbox" name="{{ field.code }}" id="{{ field.code ~ '_' ~ loop.index }}">
                    <label class="form-check-label" for="{{ field.code ~ '_' ~ loop.index }}">{{ field.title }}</label>
                </div>
            ...
            {% elseif field.type == 'multicheckbox' %}
                <div class="form-group form-check field-{{ field.code }}">
                    ...
                    {% for option in field.options %}
                        <label class="radio-inline">
                            <input type="checkbox" name="{{ field.code }}[]" value="{{ option.value }}" id="{{ field.code ~ '_' ~ loop.index }}">
                            {{ option.title }}
                        </label>
                    {% endfor %}
                </div>
            ...
            {% elseif field.type == 'file' %}
                <div class="form-group form-check field-{{ field.code }}">
                    <input class="form-check-input" type="file" name="{{ field.code }}" id="{{ field.code ~ '_' ~ loop.index }}">
                    <label class="form-check-label" for="{{ field.code ~ '_' ~ loop.index }}">{{ field.title }}</label>
                </div>
            {% endif %}
        {% endfor %}
